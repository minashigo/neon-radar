...
def test_trade_setup_engine_bullish() -> None:
    ...
    from neon_radar.domain.indicators.base import IndicatorSeries, IndicatorKind
    atr_series = IndicatorSeries(
        name="atr_14",
        kind=IndicatorKind.META,
        snapshots=tuple([{"atr": 10.0}] * 3)
    )
...
def test_trade_setup_engine_neutral_returns_none() -> None:
    ...
    state = MarketState(symbol=Symbol("BTCUSDT"), timestamp=0, primary_series=series, indicator_series=())
