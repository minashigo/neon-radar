...
async def main():
    ...
    async with BinanceClient(cfg.api) as client:
        ...
        for name, (start_str, end_str) in PERIODS.items():
            ...
            start_ms = (
                datetime.strptime(start_str, "%Y-%m-%d").replace(tzinfo=UTC).timestamp() * 1000
            )
            ...
            for tf in ["1d", "4h"]:
                ...
                regime_trades = [t for t in global_trades[tf] if start_ms <= t.entry_time <= end_ms]
        ...
        with open("results/trades_export.csv", "w", newline="", encoding="utf-8") as f:
            ...
            for tf, trades in global_trades.items():
                for t in trades:
                    regime = (
                        t.diagnostics.regime
                        if t.diagnostics and hasattr(t.diagnostics, "regime")
                        else "UNKNOWN"
                    )
                    reason = (
                        t.diagnostics.regime_reason
                        if t.diagnostics and hasattr(t.diagnostics, "regime_reason")
                        else ""
                    )
                    ...
                    writer.writerow(
                        [
                            ...
                            datetime.fromtimestamp(t.exit_time / 1000, tz=UTC).isoformat()
                            if t.exit_time
                            else "",
                            t.entry_price,
                            t.exit_price,
    ...
    print(
        "WFA regime analysis completed. Results saved to results/phase1_wfa_regime_summary.json and results/trades_export.csv"
    )
