...
class ScoringRulesConfig(BaseModel):
    ...
    confluence_bonus: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Confidence bonus per confirming category."
    )
    confluence_penalty: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Confidence penalty per conflicting category."
    )
