...
class PortfolioEngine:
    def __init__(self, initial_capital: float, currency: str = "USDT") -> None:
        account = AccountState(total_capital=initial_capital, free_capital=initial_capital, currency=currency)
    ...
    def update_market_prices(self, symbol_prices: dict[str, float]) -> None:
        ...
        account = AccountState(
            total_capital=self._state.account.free_capital + sum(p.position_size for p in updated_positions) + total_unrealized,
    ...
    def close_position(self, symbol: str, exit_price: float, closed_at: int, reason: str = "MANUAL") -> ClosedPosition | None:
        target_pos = None
        remaining_positions = []
        for pos in self._state.positions:
        ...
        if target_pos.direction == Bias.BULLISH:
            realized_pnl = (exit_price - target_pos.entry_price) * target_pos.quantity
        else:
            realized_pnl = (target_pos.entry_price - exit_price) * target_pos.quantity
        ...
        closed = ClosedPosition(
            ...
            fees=target_pos.fees,
            funding=target_pos.funding_paid,
            close_reason=reason,
        ...
        winning_trades = stats.winning_trades + (1 if realized_pnl > 0 else 0)
        losing_trades = stats.losing_trades + (1 if realized_pnl <= 0 else 0)
        gross_profit = stats.gross_profit + (realized_pnl if realized_pnl > 0 else 0)
        gross_loss = stats.gross_loss + (abs(realized_pnl) if realized_pnl <= 0 else 0)

        # Balance = old free capital + position margin + realized PnL
        new_free = self._state.account.free_capital + target_pos.position_size + realized_pnl
