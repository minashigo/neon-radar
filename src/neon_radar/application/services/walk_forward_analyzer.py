...
class WalkForwardAnalyzer:
    ...
    async def run(
        ...
        # The base_backtester.run on the full period with no rules will prefetch everything to its cache.
        # But `run` executes trades. A better way is to call _prefetch.
        ...
        while True:
            ...
            if current_oos_start >= end_date:
                # No space left for an OOS window
                break

            # If the end of OOS exceeds total end_date, we cap it or we can skip.
            # We'll cap it so we use all available data.
            if current_oos_end > end_date:
                current_oos_end = end_date

            logger.info(
                f"WFA Cycle: IS [{current_is_start} to {current_is_end}] -> "
            ...
            oos_trades = await oos_tester.run(
                ...
                symbols=symbols,
                timeframe=timeframe,
                # min_history is handled automatically via caching
            )
